...
public static unsafe class MidInterpreter3
{
    ...
    internal static extern void Coplt_MidInterpreter_Exec(
        IntPtr root_func,
        Regs* regs,
        byte* stack_data,
        Frame* stack_frames,
        uint stack_data_len,
        uint stack_frames_len
    );

    public static R CallFunc<R, A0>(MidFunc3 func, A0 a0)
        where R : unmanaged
        where A0 : unmanaged
    {
        Regs regs = default;
        ...
        fixed (Frame* p_frames = stack.Frames)
        {
            foreach (ref readonly var parameter in func.m_parameters.AsSpan())
            {
                if (parameter.Place is SigLoc.Local) throw new NotImplementedException("todo");
                else regs.Set(parameter.Place, a0);
            }
            Coplt_MidInterpreter_Exec(func.m_native.m_ptr, &regs, p_data, p_frames, (uint)stack.Data!.Length, (uint)stack.Frames!.Length);
